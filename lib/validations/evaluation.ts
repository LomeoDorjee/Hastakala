import * as z from 'zod'

export const MarksValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
    hod_q1: z.number().lte(10).nonnegative(),
    hod_q2: z.number().lte(10).nonnegative(),
    hod_q3: z.number().lte(10).nonnegative(),
    hod_q4: z.number().lte(10).nonnegative(),
    hod_q5: z.number().lte(10).nonnegative(),
    sup_q6: z.number().lte(10).nonnegative(),
    sup_q7: z.number().lte(10).nonnegative(),
    sup_q8: z.number().lte(10).nonnegative(),
    sup_q9: z.number().lte(10).nonnegative(),
    sup_q10: z.number().lte(10).nonnegative(),
    sup_q1: z.number().lte(5).nonnegative(),
    sup_q2: z.number().lte(5).nonnegative(),
    sup_q3: z.number().lte(5).nonnegative(),
    sup_q4: z.number().lte(5).nonnegative(),
    sup_q5: z.number().lte(5).nonnegative()
})

export const AppraisalValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
})

export const EducationValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
    qualification: z.string(),
})

export const LeaveEvalValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
    category: z.string(),
    point: z.number(),
})

export const AppreciationValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
    category: z.string(),
    point: z.number(),
})

export const WarningValidation = z.object({
    staffid: z.number(),
    fyearid: z.number(),
    category: z.string(),
    point: z.number(),
})