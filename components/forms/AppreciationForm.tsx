...
import { AppraisalValidation, AppreciationValidation, EducationValidation, LeaveEvalValidation } from "@/lib/validations/evaluation"
import { createUpdateAnnualLeaveData, createUpdateAppraisal, createUpdateAppreciation, createUpdateEducation } from "@/lib/actions/performance/evaluation.actions"
...
export default function AppreciationForm({
    ...
    const formSubmit = async (formData: FormData) => {
        ...
        // Validation
        const zod = AppreciationValidation.safeParse(newData)
        if (!zod.success) {
        ...
        const response = await createUpdateAppreciation(zod.data)
        if (response?.error) {
        ...
        <>
            <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={false} backdrop="blur" size="lg">
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">Appreciation Actions</ModalHeader>
                            <ModalBody>
                                <form
                                    action={formSubmit}
