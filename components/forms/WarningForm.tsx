...
import { AppraisalValidation, AppreciationValidation, EducationValidation, LeaveEvalValidation, WarningValidation } from "@/lib/validations/evaluation"
import { createUpdateAnnualLeaveData, createUpdateAppraisal, createUpdateAppreciation, createUpdateEducation, createUpdateWarning } from "@/lib/actions/performance/evaluation.actions"
...
export default function WarningForm({
    ...
    const formSubmit = async (formData: FormData) => {
        ...
        // Validation
        const zod = WarningValidation.safeParse(newData)
        if (!zod.success) {
        ...
        const response = await createUpdateWarning(zod.data)
        if (response?.error) {
        ...
        <>
            <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={false} backdrop="blur" size="lg">
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">Warning Actions</ModalHeader>
                            <ModalBody>
                                <form
                                    action={formSubmit}
